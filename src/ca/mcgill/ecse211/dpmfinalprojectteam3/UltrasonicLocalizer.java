...
public class UltrasonicLocalizer {
	...
	public static boolean WRONG_SIDE = false;
	 ...
	 * values
	 */
	private static final int BOTTOM_THRESHOLD = 30; // "d-k"
	...
	public void doLocalization() {
		...
		if (localizationType == LocalizationType.RISINGEDGE) { // Starts by facing the wall
			...
			if (dist > BOTTOM_THRESHOLD || dist ==0 ) {
				this.dist = poller.getReading();
				while(this.dist > BOTTOM_THRESHOLD || dist ==0){
					this.dist = poller.getReading();
				}
				WRONG_SIDE = true;
			}
			...
			while (this.dist < BOTTOM_THRESHOLD) { // It's seeing the wall during the turn
				...
				if (this.dist > BOTTOM_THRESHOLD) {
					FIRST_ANGLE = odometer.getTheta(); // Stop the motors when it doesn't see the wall anymore
	...
	public void updateAngle(double first_angle, double second_angle) {
		...
		if (localizationType == LocalizationType.FALLINGEDGE) {
			...
			if(WRONG_SIDE == false) {
				UPDATED_ANGLE = UPDATED_ANGLE - 175; // This value is found experimentally
				navigation.turn(UPDATED_ANGLE);
			}
			else {
				UPDATED_ANGLE = UPDATED_ANGLE + 355; // This value is found experimentally
				navigation.turn(UPDATED_ANGLE);
			}
