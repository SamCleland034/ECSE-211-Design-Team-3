					...
					while (FinalProject.usMotor.isMoving()) {
						...
						if(distance < 40) {
							...
							while(distance > 4) { // make the robot go to the block and stop when it gets too close
								...
								FinalProject.leftMotor.forward();
								FinalProject.rightMotor.forward();
								if(FinalProject.odometer.getX() - X > 20 && FinalProject.odometer.getY() - Y > 20) { //break if it went too far and didnt recognize anything
									break;
								}
							}
							FinalProject.leftMotor.stop(true);
							FinalProject.rightMotor.stop(false);
							//LightPoller.checkColors(correctColors); //change the lightPoller to colorPoller (or not idk)
				...
				if (detect ==false) {
					...
					while (FinalProject.usMotor.isMoving()) {
						...
						if(distance < 40) {
							...
							FinalProject.leftMotor.stop(true);
							FinalProject.rightMotor.stop(false);
							//LightPoller.checkColors(correctColors);
							if (colorpoller.checkColors(correctColors)) {
								for (int j = 0; j < 3; j++) {
									Sound.beep();
									try {
